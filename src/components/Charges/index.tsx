...
import { chargesInputData } from '../../constants/Suppliers';
...
const Charges = (props: PresetFilterProps) => {
  const {
    applyFilter,
    filterData = [],
    resetPreset,
  ...
  const [localValues, setLocalValues] = React.useState(chargesInputData);
  ...
  const setLocalData = (dataKey: string, type: string, value?: any) => {
    const filters: any[] = _.cloneDeep(localValues);
