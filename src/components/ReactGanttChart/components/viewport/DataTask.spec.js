...
describe('Testing Firing Events ', function() {
  ...
  it('Move and handle mouse event properly', () => {
    ...
    wrapper
      .instance()
      .doMouseDown({ button: 1, clientX: 10, stopPropagation: stopPropagation }, MODE_MOVE);
    expect(wrapper.state().mode).toBe(MODE_NONE);
    expect(wrapper.state().dragging).toBe(false);
    wrapper
      .instance()
      .doMouseDown({ button: 0, clientX: 10, stopPropagation: stopPropagation }, MODE_MOVE);
    ...
    expect(new_start_date.getTime() - onUpdateTask.mock.calls[0][1].start.getTime() < 10).toBe(
      true
    );
  ...
  it('Resize Left and handle mouse event properly', () => {
    ...
    wrapper
      .instance()
      .doMouseDown({ button: 1, clientX: 10, stopPropagation: stopPropagation }, MOVE_RESIZE_LEFT);
    expect(wrapper.state().dragging).toBe(false);
    expect(wrapper.state().mode).toBe(MODE_NONE);
    wrapper
      .instance()
      .doMouseDown({ button: 0, clientX: 10, stopPropagation: stopPropagation }, MOVE_RESIZE_LEFT);
  ...
  it('Resize Right and handle mouse event properly', () => {
    ...
    wrapper
      .instance()
      .doMouseDown({ button: 1, clientX: 10, stopPropagation: stopPropagation }, MOVE_RESIZE_RIGHT);
    expect(wrapper.state().dragging).toBe(false);
    expect(wrapper.state().mode).toBe(MODE_NONE);
    wrapper
      .instance()
      .doMouseDown({ button: 0, clientX: 10, stopPropagation: stopPropagation }, MOVE_RESIZE_RIGHT);
