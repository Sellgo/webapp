...
class SubscriptionPricing extends React.Component<Props, State> {
  ...
  render() {
    ...
    const subscribedSubscription = sellerSubscription ? this.props.subscriptions.filter(e => e.id === sellerSubscription.subscription_id)[0] : undefined;
    const header = subscribedSubscription ? `You have subscribed to ${subscribedSubscription.name} Plan` : 'Choose the one that best fits you!';

    return (
      <AdminLayout auth={this.props.match.params.auth} sellerData={this.props.sellerData} title={'Subscription'}>
              ...
              <Card key={index} style={{ display: 'inline-block', margin: '10px', verticalAlign: 'baseline' }}>
                <CardContent>
                  ...
                  <Checkout subscription={subscription}>
                    <Button
                      basic={true}
                      style={{ borderRadius: 20, background: 'rgb(66, 133, 244) !important', fontWeight: 'bold' }}
                      color="blue"
                    >{subscribedSubscription ? subscribedSubscription.id === subscription.id ? 'EXTEND PLAN' : 'CHANGE PLAN' : 'SUBSCRIBE'}</Button>
                  </Checkout>
                </CardContent>
              </Card>
            )}
        ...
        <Modals title="" size="large" open={isOpen} close={this.handleModel} bCloseIcon={true}>
          <Container textAlign="center">
            ...
            <Segment textAlign="center" basic={true}>
              <Button style={buttonStyle} content="Ok" onClick={this.handleModel} as={Link}
                to={this.message.to} />
            </Segment>
          </Container>
        </Modals>
