    ...
    if (type == 'identify') {
      try {
        ...
        idfy.onload = function() {
          if (idfy.status != 200) {
            console.log(
              'Response-Error (' + idfy.status + ') Upvoty identify: ' + idfy.responseText
            );
          }
        };
      ...
      if (is_safari) {
        if (!document.cookie.match(/^(.*;)?\s*fixed\s*=\s*[^;]+(.*)?$/)) {
          ...
          window.location.replace(
            'https://' + upvoty.settings.baseUrl + '/widget-fix-iframe-3rd.html'
          );
        }
      ...
      upvoty.iframe.style.height = '100%';
      upvoty.iframe.style.position = 'absolute';
      upvoty.iframe.style.inset = '0';
      ...
      if ((elem = document.getElementById('feedback'))) {
        ...
        for (var i = 0; i < children.length; i++) {
          if (
            ...
            children[i].className.split(' ').indexOf('section-body') >= 0
          ) {
  ...
  func_trigger_message: function(e) {
    if (e.data !== undefined) {
      ...
      try {
        ...
        if (upvoty.debug)
          console.log([
            'PARENT-RECIEVE:' + upvoty.counters.receive++,
            [inputData.method, inputData],
          ]);
        if (inputData.method == 'init') {
            ...
            if (window.scrollY > 0 && upvoty.iframe.getBoundingClientRect().top < 0) {
              var gotoPos =
                window.scrollY > 0 && upvoty.iframe.getBoundingClientRect().top < 0
                  ? upvoty.iframe.getBoundingClientRect().top +
                    window.scrollY -
                    upvoty.settings.offsetTop
                  : 0;
              if (gotoPos > 0) {
          ...
          if (!upvoty.preventScrollEvent) {
            ...
            if (
              window.scrollY > 0 &&
              newTop + upvoty.iframe.getBoundingClientRect().top != window.scrollY
            ) {
              var gotoPos =
                newTop +
                (window.scrollY > 0 && upvoty.iframe.getBoundingClientRect().top < 0
      ...
      upvoty.sendMessage({
        ...
        endOfPage:
          (upvoty.elmScrollTrigger == window &&
            document.body.scrollHeight - window.scrollY - upvoty.settings.offsetBottom <=
              window.innerHeight) ||
          upvoty.iframe.getBoundingClientRect().bottom - upvoty.settings.offsetBottom <=
            window.innerHeight,
      });
    }
  },
