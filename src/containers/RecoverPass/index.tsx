...
export default class RecoverPass extends React.Component<any, State> {
  ...
  changePassword = () => {
    ...
    if (reg.test(this.state.email) === false) {
      ...
      if (this.state.email === "") {
        this.props.isSuccessReset({ isFailed: true , errorMgs: 'Please enter email or email field is empty' })
      } else {
        this.props.isSuccessReset({ sFailed: true , errorMgs: 'Please enter valid email address.' })
      }
    } else {
      axios
        .get(URLS.BASE_URL_API + `seller/exists?email=${this.state.email}`)
        .then(response => {
          if (response.data) {
        ...
        {!this.props.onlyEmail ? (
          <Grid.Row>
            ...
            <Grid.Column className="right-pane" width={6}>
              ...
              {this.state.isFailed && (
                <Message color="red">{this.state.errorMgs}</Message>
              )}
          ...
          <Grid.Column  width={this.state.isSuccess && this.props.onlyEmail ? 16 : 14}>
            {this.state.isSuccess ? (!this.props.onlyEmail ? this.response : this.forgetPassForm) : this.forgetPassForm}
          </Grid.Column>
        )}
      </Grid>
    );
